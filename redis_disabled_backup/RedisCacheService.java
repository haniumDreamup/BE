...
public class RedisCacheService {
    ...
    public Set<String> getActiveTokens() {
        try {
            return redisTemplate.keys("token:*");
        } catch (Exception e) {
            log.error("활성 토큰 조회 실패", e);
            return null;
        }
    }
    ...
    public void revokeUserTokens(Long userId) {
        try {
            String pattern = "token:user:" + userId + ":*";
            Set<String> keys = redisTemplate.keys(pattern);
            if (keys != null && !keys.isEmpty()) {
                redisTemplate.delete(keys);
                log.info("사용자 토큰 삭제 - userId: {}, 삭제된 토큰: {}", userId, keys.size());
            }
        } catch (Exception e) {
            log.error("사용자 토큰 삭제 실패 - userId: {}", userId, e);
        }
    }
}