...
main() {
    ...
    # 1. 긴급 상황 발생 신고 - JSON 파싱 에러 (400 예상)
    ...
    test_endpoint "POST" "/api/v1/emergency/alert" "400" "긴급 상황 발생 신고 (JSON 파싱 에러)" "$emergency_data"
    ...
    # 3. 긴급 상황 상태 조회 - 찾을 수 없음 (404 예상)
    test_endpoint "GET" "/api/v1/emergency/status/1" "404" "긴급 상황 상태 조회 (찾을 수 없음)"
    ...
    # 6. 긴급 상황 해결 처리 - 잘못된 파라미터 (400 예상)
    test_endpoint "PUT" "/api/v1/emergency/1/resolve?resolvedBy=테스트&notes=해결" "400" "긴급 상황 해결 처리 (잘못된 파라미터)"
